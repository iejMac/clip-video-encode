...
with open(os.path.join(args.shards, "splits.csv"), "a+", newline='') as f:
    csv_writer = writer(f)
    csv_writer.writerow(["tar_file", "split"])

# This is the output pattern under which we write shards.
pattern = os.path.join(args.shards, f"ds_%06d.tar" + (".gz" if args.compression else ''))
with wds.ShardWriter(pattern, maxsize=int(args.maxsize), maxcount=int(args.maxcount)) as sink:
    for split in SPLITS:
        ...
        npy_files = {npy_file.stem: npy_file for npy_file in npy_files}
        npy_total = len(npy_files)

        print('Found {:,} textfiles and {:,} numpy files.'.format(text_total, npy_total))
        keys = (npy_files.keys() & text_files.keys())
        ...
        tar_split = [("ds_%06d" % (tar_count + i), split) for i in range(split_tar_count)]
        with open(os.path.join(args.shards, "splits.csv"), "a+", newline='') as f:
            csv_writer = writer(f)
        ...
        for i in indexes:
            ...
            ds_key = str(text_files[keys[i]]).split("_")[-1]
